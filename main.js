...
mobile: MethodMobile, 
browser: opcion == '1' ? ['IgnaBot-MD', 'Edge', '2.0.0'] : methodCodeQR ? ['IgnaBot-MD', 'Edge', '2.0.0'] : ['Ubuntu', 'Edge', '110.0.1587.56'],
auth: {
creds: state.creds,
...
conn.sdemote = '*𝙃𝙚𝙮 @user 𝘿𝙀𝙅𝘼𝙔𝙖 𝙣𝙤 𝙚𝙧𝙚𝙨 𝙖𝙙𝙢𝙞𝙣😐!!*'
...
console.log(chalk.bold.green(`\n🟡 IgnaJadiBot 🟡 │ NADA POR ELIMINAR 🗑️♻️`))
} else {
console.log(chalk.bold.cyanBright(`\n⚪ IgnaJadiBot ⚪ │ ARCHIVOS NO ESENCIALES ELIMINADOS 🗑️♻️`))
}} catch (err) {
console.log(chalk.bold.red(`\n🔴 IgnaJadiBot 🔴 │ OCURRIÓ UN ERROR 🗑️♻️\n` + err))
}}
...
if (err) {
console.log(chalk.bold.red(`\n🔴 ARCHIVO 🔴\n│ ${file} NO SE LOGRÓ BORRAR 🗑️❌\n` + err))
} else {
console.log(chalk.bold.green(`\n🟣 ARCHIVO │ ${file} BORRADO CON ÉXITO 🗑️♻️`))
} }) }
}) }) }) }
...
await clearTmp()
console.log(chalk.bold.cyanBright(`\n🟢 MULTIMEDIA 🟢 │ ARCHIVOS DE LA CARPETA TMP ELIMINADAS 🗑️♻️`))}, 1000 * 60 * 4) // 4 min 

//setInterval(async () => {
//if (stopped === 'close' || !conn || !conn.user) return
//await purgeSession()
//console.log(chalk.bold.cyanBright(`\n🔵 ${global.authFile} 🔵 │ SESIONES NO ESENCIALES ELIMINADAS 🗑️♻️`))}, 1000 * 60 * 10) // 10 min
...
if (stopped === 'close' || !conn || !conn.user) return
await purgeOldFiles()
console.log(chalk.bold.cyanBright(`\n🟠 ARCHIVOS 🟠 │ ARCHIVOS RESIDUALES ELIMINADAS 🗑️♻️`))}, 1000 * 60 * 10)
