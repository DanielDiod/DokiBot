...
conn.sendFile(m.chat, r2es, 'error.mp4', `*DESCARGA EXITOSA ✅*\n\n${wm}`, m);
} catch (err1) {
try {
...
conn.sendFile(m.chat, VIDEO, 'error.mp4', `*DESCARGA EXITOSA ✅*\n\n${wm}`, m);
} catch (err2) {
try {
const ress = await fg.fbdl(args[0]);
const urll = await ress.data[0].url;
await conn.sendFile(m.chat, urll, 'error.mp4', `*DESCARGA EXITOSA ✅*\n\N${wm}`, m);
} catch (err3) {
try {
const res = await fbDownloader(args[0]);
for (const result of res.download) {
const ur = result.url;
await conn.sendFile(m.chat, ur, 'error.mp4', `DESCARGA EXITOSA ✅*\n\n${wm}`, m);
}
} catch (err4) {
try {
...
await conn.sendFile(m.chat, url3, 'error.mp4', `*DESCARGA EXITOSA ✅*\n\n${wm}`, m);
} catch (err5) {
try {
...
await m.reply(`${lenguajeGB['smsAvisoFG']()}*[❌ ERROR ❌]*\n\n*AL PARECER ALGO SALIO MAL VERIFIQUE SU ENLACE DE FACEBOOK*`)
handler.limit = 0
}}}}}}}}
