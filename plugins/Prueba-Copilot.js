...
const isCommand1 = /^(googlnnnnnef?)$/i.test(command)
const isCommand2 = /(openainnnai)/i.test(command)
const isCommand3 = /^(jjbot|jjsimi|simnnne)$/i.test(command)
const isCommand4 = /^(githubstalkkkkk)$/i.test(command)
const isCommand5 = /^(ythh(s|search))$/i.test(command)
const isCommand6 = /^(trahhhnslate|traducirtttr|tradddd)$/i.test(command)
const isCommand7 = /^(openaivozzzz|chatgptvozzzz)$/i.test(command)
const isCommand8 = /^(gemiiiiini|barddddd)$/i.test(command)
const isCommand9 = /^(bing|bingia|iabing|copilot)$/i.test(command)
    
let fkontak = { "key": { "participants":"0@s.whatsapp.net", "remoteJid": "status@broadcast", "fromMe": false, "id": "Halo" }, "message": { "contactMessage": { "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:y\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD` }}, "participant": "0@s.whatsapp.net" }
async function reportError(e) {
await m.reply('Vuelva a intentarlo.')
...
try {
let syms = `Actuaras como un Bot de WhatsApp el cual fue creado por Daniel, tu seras Shana`
...
handler.command = /^(gemini|bard|bing|bingia|iabing|copilot)$/i
