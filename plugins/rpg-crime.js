...
if (media === 'text3') return m.reply(`〔🪙〕*${pickRandom(global.robar)}*\n\n${diamond} 🪙 EUROSn${money} 🪙 IGNACOINS`).catch(global.db.data.users[m.sender].limit += diamond).catch(global.db.data.users[m.sender].money += money)
if (media === 'text4') return m.reply(`〔🚓〕${pickRandom(global.robmal)}\n\n${diamond} 🪙 EUROS\n${money} 🪙 IGNACOINS`).catch(global.db.data.users[m.sender].limit -= diamond).catch(global.db.data.users[m.sender].money -= money) 
}
handler.help = ['robar'];
handler.tags = ['xp'];
