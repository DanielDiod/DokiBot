...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  try {
    ...
    if (res) {
      if (!global.applemusicList) {
        global.applemusicList = [];
      }    
      if (global.applemusicList[0]?.from == m.sender) {
        global.applemusicList.splice(0, global.applemusicList.length);
      }    
      global.applrmusicList = resultados.map(v => `${v.nombre} - ${v.artistas.join(', ')}`)
