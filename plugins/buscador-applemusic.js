...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  try {
    let resultados = await applemusicxv(text)
    ...
    if (res) {
      if (!global.apple.musicList) {
        global.apple.musicList = [];
      }    
      if (global.apple.musicList[0]?.from == m.sender) {
        global.apple.musicList.splice(0, global.apple.musicList.length);
      }    
      global.applr.musicList = resultados.map(v => `${v.nombre} - ${v.artistas.join(', ')}`)
